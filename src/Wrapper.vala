...
namespace ToxWrapper {
  ...
  public class Tox : Object {
    ...
    public signal void group_invite (uint32 friend_number, uint8 type, uint8[] pubkey);
    ...
    public Tox (ToxCore.Options? opts = null, string? profile = null, string? password = null, bool is_new = false) throws ErrNew, ErrDecrypt {
      ...
      handle.callback_friend_name ((self, num, name) => {
        ...
        if (old_name != new_name) {
          this.friends[num].friend_info (old_name, new_name);
      ...
      this.handle.callback_group_invite((self, friend_number, type, pubkey) => {
        this.group_invite(friend_number, type, pubkey);
      });
      this.handle.callback_group_message((self, group_number, peer_number, message) => {
        this.groups[group_number].message(peer_number, Util.arr2str(message));
      });
    ...
    public Group join_groupchat(int32 friend_number, uint8 type, uint8[] pubkey) {
      int group_number;
      if (type == 0/*TOX_GROUPCHAT_TYPE_TEXT*/) {
        debug("join text type groupchat");
        group_number = this.handle.join_groupchat(friend_number, pubkey);
      } else if (type == 1/*TOX_GROUPCHAT_TYPE_AV*/) {
        debug("join av type groupchat");
        group_number = this.handle.join_av_groupchat(friend_number, pubkey, null);
      } else {
        group_number = -1;
      }
      var group = new Group(this, group_number);
  ...
  public class Friend : Object {
    ...
    public signal void friend_info (string old_name, string new_name);
    ...
    public DateTime last_online (string? format) {
      uint64 last = this.tox.handle.friend_get_last_online (this.num, null);
      //debug (@" for $num: $last");

      return new DateTime.from_unix_local ((int64)last);
  ...
  public class Group : Object {
    ...
    public signal void message (int peer_number, string message);
    ...
    public bool send_message (string message) {
      return (bool) this.tox.handle.group_message_send (this.num, message.data);
    }
  }
