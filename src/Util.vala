...
namespace Util {
  ...
  public static string status_to_icon (ToxWrapper.UserStatus status, int messagesCount = 0) {
    string icon = "";

    switch (status) {
      case ToxWrapper.UserStatus.BLOCKED:
        icon = (messagesCount > 0) ? "invisible" : "invisible";
        break;
      case ToxWrapper.UserStatus.ONLINE:
        icon = (messagesCount > 0) ? "online_notification" : "online";
        break;
      case ToxWrapper.UserStatus.AWAY:
        icon = (messagesCount > 0) ? "idle_notification" : "idle";
        break;
      case ToxWrapper.UserStatus.BUSY:
        icon = (messagesCount > 0) ? "busy_notification" : "busy";
        break;
      case ToxWrapper.UserStatus.OFFLINE:
      default:
  ...
  /*public static string status_to_string (ToxWrapper.UserStatus status) {
    string str = "";
